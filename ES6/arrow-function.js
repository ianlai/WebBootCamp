...
*/
 let tripleAndFilter = arr => arr.map(v => v*3).filter(v=>v%5===0);
...
*/
 let doubleOddNumbers = arr => arr.filter(v=>v%2!==0).map(v=>v*2);
...
*/
 let mapFilterAndReduce = arr => arr.map(v=>v.firstName).filter(v=>v.length<5).reduce((acc,next)=>{
         acc[next]=next.length;
         return acc;
     },{});
...
//     return obj;
// }
var createStudentObj = (f,l) => ({firstName:f, lastName:l});
